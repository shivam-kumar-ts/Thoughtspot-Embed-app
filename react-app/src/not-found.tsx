...
import { NAV_ITEMS, PAGE_TEXT } from "./utils/constants";
  ...
  return (
    <div className={classes.notFound}>
      ...
      <div className={classes.notFoundActions}>
        <Link to="/" className={classes.notFoundBtn}>
          {PAGE_TEXT.NOT_FOUND.BACK_HOME}
        </Link>
        <Link to="/liveboard" className={classes.notFoundBtnSecondary}>
      ...
      <div className={classes.notFoundLinks}>
        {NAV_ITEMS.map((p) => (
          <Link key={p.href} to={p.href} className={classes.notFoundChip}>
            {p.label} &rarr;
          </Link>
        ))}
      </div>
